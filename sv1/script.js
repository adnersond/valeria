...
clickSound.preload = 'auto'; // Forzamos la carga
...
// Función optimizada para sonar en móviles
function playMagicalSound() {
    if (!audioUnlocked) {
        // La primera vez, intentamos reproducir el sonido original
        // Esto desbloquea el audio para el resto de la sesión
        clickSound.play().then(() => {
            audioUnlocked = true;
        }).catch(() => {
            // Si falla, lo intentará en el siguiente click
        });
    }

    // Creamos un clon para sonidos rápidos superpuestos
    const soundClone = clickSound.cloneNode();
    soundClone.volume = 0.2;
    soundClone.play().catch(() => {}); // Ignoramos errores si el clon falla
}
...
function startAction(e) {
    isDragging = true;
    playMagicalSound(); // Intentamos desbloquear/sonar inmediatamente
...
function moveAction(e) {
    ...
    const y = e.clientY || (e.touches && e.touches[0].clientY);
    handleInteraction(x, y);
    if (Math.random() > 0.94) playMagicalSound();
}
